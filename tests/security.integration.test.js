...
test('L3 override requires JWT and logs fingerprint when token is valid', () => {
    ...
    const mockFs = {
        readFileSync(filePath) {
            assert.equal(filePath, 'public.pem');
...
test('L3 override fails closed when override flag has no token', () => {
    ...
    try {
        const { engageL3Override } = loadWithMocks('cli/l3-override.js', {
            jsonwebtoken: mockJwt,
            fs: { readFileSync() { return 'unused'; }, appendFileSync() {} },
        });
...
test('L3 override fails closed when public.pem is missing', () => {
    const originalArgv = process.argv;
    const originalExit = process.exit;
    process.argv = ['node', 'cli/l3-override.js', '--admin-override', '--token=valid.jwt.token'];
    ...
    try {
        const { engageL3Override } = loadWithMocks('cli/l3-override.js', {
            ...
            fs: {
                ...
                appendFileSync() {
                    throw new Error('must not write audit without key');
                },
            },
        });
...
test('AI Bridge fails closed when L3 token is missing', () => {
    const originalExit = process.exit;
    let exitCode;
    process.exit = (code) => {
        exitCode = code;
    ...
    try {
        const { verifyBridgeAccessOrExit } = loadWithMocks('engine/ai-bridge.js', {
            playwright: { chromium: {} },
            jsonwebtoken: { verify() { return {}; } },
            fs: { readFileSync() { return 'unused'; } },
            '../ai/build-secure-sandbox': { buildSecureSandbox() {} },
        });

        assert.throws(() => verifyBridgeAccessOrExit(''), /exit:1/);
        assert.equal(exitCode, 1);
    } finally {
        process.exit = originalExit;
    }
...
test('AI Bridge extracts GET JSON schema and triggers secure sandbox generation once', async () => {
    const generated = [];
    let responseHandler;
    ...
    const page = {
        ...
        async goto() {
            await responseHandler({
                request: () => ({ method: () => 'GET' }),
                headers: () => ({ 'content-type': 'application/json; charset=utf-8' }),
                text: async () => JSON.stringify({ user: 'Alice', age: 30, active: true }),
    ...
    const { startAiBridge } = loadWithMocks('engine/ai-bridge.js', {
        ...
        '../ai/build-secure-sandbox': {
            buildSecureSandbox(jsonText) {
                generated.push(JSON.parse(jsonText));
            },
        },
    });

    await startAiBridge({ targetUrl: 'https://target.local', l3Token: 'valid.jwt.token' });

    assert.equal(generated.length, 1);
    assert.equal(generated[0].routePath, '/api/profile');
    assert.equal(generated[0].method, 'GET');
    assert.equal(generated[0].responseSchema.user, 'string');
    assert.equal(generated[0].responseSchema.age, 'number');
    assert.equal(generated[0].responseSchema.active, 'boolean');
});

test('AI Bridge fails closed when no valid JSON blueprint is captured', async () => {
    const originalExit = process.exit;
    process.exit = (code) => { throw new Error(`exit:${code}`); };
    ...
    try {
        ...
        const { startAiBridge } = loadWithMocks('engine/ai-bridge.js', {
            ...
            jsonwebtoken: { verify() { return { role: 'admin' }; } },
            fs: { readFileSync() { return '-----BEGIN PUBLIC KEY-----\nMOCK\n-----END PUBLIC KEY-----'; } },
            '../ai/build-secure-sandbox': { buildSecureSandbox() {} },
        });

        await assert.rejects(
            () => startAiBridge({ targetUrl: 'https://target.local', l3Token: 'valid.jwt.token' }),
            /exit:1/
        );
    } finally {
        process.exit = originalExit;
...
test('AI Bridge rejects unsafe route paths from intercepted URLs', () => {
    const { extractBlueprintFromJson } = loadWithMocks('engine/ai-bridge.js', {
        playwright: { chromium: {} },
        jsonwebtoken: { verify() { return {}; } },
        fs: { readFileSync() { return 'unused'; } },
        '../ai/build-secure-sandbox': { buildSecureSandbox() {} },
    ...
    assert.throws(
        () => extractBlueprintFromJson('https://target.local/api/evil%27);console.log(1);//', 'GET', { ok: true }),
        /INVALID_ROUTE_PATH/
    );
});
